...
class AreaController extends BaseauthController {
    ...
    public function edit() {
        if (IS_POST){
            ...
            $data= array('id' =>I('post.id'),'area_name'=>I('post.area_name'));  
            if(M('Area')->where($where)->save($data)){
                $this->success('修改成功！');
            }else{
                $this->error('修改失败！');
            }   
        }else{
            $areainfo=M('Area')->find(I('get.id'));
   ...
   /**
    ...
    public function del(){
        ...
        $areainfo=M('Area')->where($where)->delete();
        if($areainfo){  
            $this->success('删除成功！');
        }else{
            $this->error('删除失败！');
